...
// import { useGetAllTrainersQuery } from "@/app/services/trainersAPI";
// import {
//   Download,
//   Edit,
//   Mail,
...
//   const { data: apiResponse, isLoading, isError } = useGetAllTrainersQuery();
...
//     const s = status?.toUpperCase() || "PENDING APPROVAL";
//     if (s === "ACTIVE")
...
//                 role="button"
//                 tabIndex={0}
//                 className="btn bg-white border-slate-200 gap-2 min-w-[160px]"
//               >
...
//               <label
//                 tabIndex={0}
//                 className="btn bg-white outline-none gap-2 min-w-[140px]"
//               >
//                 Price:{" "}
...
//                   <TrainerCard key={trainer._id} trainer={trainer} onEditClick={() => setSelectedTrainerId(trainer._id)} />
//                 ))}
//               </div>
//             </>
//           )}
...
// function TrainerCard({ trainer, onEditClick }: { trainer: Trainer; onEditClick: () => void }) {
...
//   return (
//     <div className="card bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
//       <div className="p-6">
...
// export default function TrainersPage() {
//   const { data: apiResponse, isLoading, isError, refetch } =
...
//           refetch(); // refreshes the list after successful edit
//         }}
//       />
//     </div>
//   );
...
//               </h3>
//               <span
//                 className={`px-2.5 py-1 rounded-full text-xs font-semibold border ${getStatusStyles(
//                   status
//                 )}`}
//               >
//                 ● {status}
...
export default function TrainersPage() {
  const { data: apiResponse, isLoading, isError, refetch } = useGetAllTrainersQuery();
  ...
  const trainers: Trainer[] = (() => {
    ...
    const response = apiResponse as any;
    if (response?.data && Array.isArray(response.data)) return response.data;
    if (response?.trainers && Array.isArray(response.trainers)) return response.trainers;
    return [];
  })();
  ...
  const filteredTrainers = trainers.filter((trainer) => {
    ...
    const matchesSubject =
      subjectFilter === "ALL" ||
      trainer.subjects?.toLowerCase() === subjectFilter.toLowerCase() ||
      trainer.skills.some((s) => s.toLowerCase() === subjectFilter.toLowerCase());
    ...
    <div className="drawer lg:drawer-open bg-[#F8FAFC]">
      ...
      <div className="drawer-content flex flex-col">
        <div className="p-4 md:p-6 lg:p-8 min-h-screen">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-slate-800">Trainers</h1>
              <p className="text-sm text-slate-500 mt-1">
          ...
          <div className="flex flex-row sm:flex-row flex-wrap gap-3 mb-6">
            ...
            <div className="dropdown dropdown-end relative">
              ...
              >
                <SlidersHorizontal size={16} />
                Subjects/Skills: {subjectFilter === "ALL" ? "All" : subjectFilter}
                <ChevronDown size={14} />
              </label>
            ...
            <div className="dropdown dropdown-end">
              <label
                ...
                className="btn bg-white outline-none gap-2 min-w-[140px]"
              >
                Price: {priceOptions.find((o) => o.value === priceFilter)?.label || "All"}
                <ChevronDown size={14} />
              </label>
              <ul
                tabIndex={0}
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 text-slate-500">
              <p className="text-lg font-medium">No trainers found</p>
              <p className="mt-2">Try adjusting filters or add a new trainer.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
              {filteredTrainers.map((trainer) => (
      ...
      <div className="p-6">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <div className="mt-1 flex items-center gap-4 text-sm text-slate-600">
              <span className="font-semibold text-blue-600 text-xs">#{trainer.trainerId}</span>
              <div className="flex items-center gap-1.5">
                <Mail size={14} className="text-slate-400" />
                <span className="truncate max-w-[180px]">{trainer.email}</span>
              </div>
            ...
            {trainer.subjects && trainer.subjects.trim() && (
              <div className="mt-1.5 text-sm text-slate-600">
                <span className="font-medium">Subjects:</span> {trainer.subjects}
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-2 gap-y-5 gap-x-6 mb-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-base font-semibold text-slate-800">{hourlyDisplay}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-base font-semibold text-slate-800">{trainer.phone}</p>
            </div>
          </div>
        </div>
        ...
        {trainer.description && trainer.description.trim() && (
          <div className="mb-6 text-sm text-slate-600">
            ...
            <p className="leading-relaxed line-clamp-4">{trainer.description}</p>
          </div>
        )}
